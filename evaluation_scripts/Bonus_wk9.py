...
column_weeks = [i for i in weekly_forecast1w_graph.T.columns]
week_flows = weekly_flows.iloc[1:len(column_weeks) + 1, 3:4]
week_flows.set_index(
    weekly_forecast1w_graph.T.columns, append=False, inplace=True)
...
for i in range(nstudent):
    err[i] = (obs_var[0:len(column_weeks)+1] -
              pred_var[i][0:len(column_weeks)-1])
...
for i in range(1, 4):
    ...
    weekly_winners = weekly_winners.append(
        {'Week': 1, 'Names': summary.loc[summary['1week_ranking'] == i].index[0]}, ignore_index=True)
    weekly_winners = weekly_winners.append(
        {'Week': 2, 'Names': summary.loc[summary['2week_ranking'] == i].index[0]}, ignore_index=True)

print('The weekly winners are: \n', weekly_winners)
...
while Bonus_winners[Bonus_winners.Names.isin(weekly_winners['Names'])].any()['Names'] == True:
    ...
    Bonus_winners.drop(Bonus_winners[Bonus_winners.Names.isin(
        weekly_winners['Names']) == True].index, inplace=True)

    # Add the next person in the list
    Bonus_winners = Bonus_winners.append(
        max_error_df.head().iloc[i:i+(3-Bonus_winners.shape[0])])
    i = i+1

print('The Bonus winners for this week are:\n', Bonus_winners.Names)

# Write the bonus points into the bonus points file
ev_fn.write_bonus(Bonus_winners.Names, len(column_weeks)+1)
